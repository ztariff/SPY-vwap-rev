...
.refresh-status{font-size:11px;color:#555}
.refresh-btn{background:#1a1e2e;color:#aaa;border:1px solid #3a3f52;border-radius:6px;padding:7px 16px;font-size:12px;cursor:pointer;font-weight:600;transition:all .2s;white-space:nowrap}
.refresh-btn:hover{background:#2a2e3d;color:#fff;border-color:#4a5068}
.refresh-btn.running{border-color:#f59e0b88;color:#f59e0b;opacity:.7;cursor:wait}
.refresh-btn.done{border-color:#22c55e;color:#22c55e}
.refresh-btn.error{border-color:#ef5350;color:#ef5350}
</style>
</head>
<body>
...
// REFRESH DATA
let lastTradeCount = Object.values(ALL_DATA).reduce((s, arr) => s + arr.length, 0);
...
async function refreshData() {
  ...
  try {
    ...
    // Update global data
    for (const [name, trades] of Object.entries(d.trades)) {
      ALL_DATA[name] = trades;
    }
    for (const [name, meta] of Object.entries(d.meta)) {
      STRAT_META[name] = meta;
    }

    // Calculate diff
    const newCount = d.total_trades;
    const diff = newCount - lastTradeCount;
    let diffMsg;
    if (diff > 0) diffMsg = '+' + diff + ' new trade' + (diff > 1 ? 's' : '');
    else if (diff < 0) diffMsg = Math.abs(diff) + ' trade' + (Math.abs(diff) > 1 ? 's' : '') + ' removed';
    else diffMsg = 'no new trades';
    lastTradeCount = newCount;

    status.textContent = 'Last refreshed ' + d.timestamp + ' \u00b7 ' + newCount + ' trades total \u00b7 ' + diffMsg;
    status.style.color = diff > 0 ? '#22c55e' : '#555';

    btn.className = 'refresh-btn done';
    btn.innerHTML = '&#x21bb; Refresh Data';
    setTimeout(() => { btn.className = 'refresh-btn'; }, 3000);
    ...
    btn.innerHTML = '&#x21bb; Refresh Data';
    setTimeout(() => { btn.className = 'refresh-btn'; }, 5000);
  }
}
