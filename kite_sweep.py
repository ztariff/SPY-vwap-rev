#!/usr/bin/env python3
"""
KITE VWAP Mean Reversion — Exhaustive Local Parameter Sweep
============================================================
Uses KITE Liberator minute bars (RTH only, 9:30-16:00 ET).
...
MINUTES_DIR = os.path.join(BASE_DIR, 'kite_data', 'minutes')
DAILY_FILE = os.path.join(BASE_DIR, 'kite_data', 'spy_daily.csv')
OUTPUT_FILE = os.path.join(BASE_DIR, 'kite_sweep_results.json')
SIGNALS_FILE = os.path.join(BASE_DIR, 'kite_signals.json')
...
def load_minute_data():
    """Load all monthly minute CSVs, filter to RTH."""
    files = sorted([
        os.path.join(MINUTES_DIR, f)
        for f in os.listdir(MINUTES_DIR)
        if f.endswith('.csv')
    ])
    if not files:
        print("ERROR: No minute CSV files found in", MINUTES_DIR)
        sys.exit(1)
    ...
    for f in files:
        df = pd.read_csv(f)
        frames.append(df)
        print(f"  {os.path.basename(f):>20s}: {len(df):>6,} bars")
    ...
    # RTH filter
    data = data[(data['time'] >= RTH_START) & (data['time'] < RTH_END)].copy()
...
def compute_wilder_atr(daily_df, target_date, period=ATR_PERIOD):
    """14-day Wilder's smoothed ATR from closes prior to target_date."""
    prior = daily_df[daily_df['date'] < target_date].tail(period + 1)
    ...
    for entry_pct in entry_pcts:
        for direction in directions:
            for i in range(MIN_BARS_FOR_VWAP, eod_idx):
                ...
                end = min(eod_idx + 5, n)  # allow a few bars past cutoff for exits
...
def precompute_exits(signal, target_pcts, stop_pcts):
    """
    For each target and stop level, find the first bar index (1-based)
    where that level is hit. Returns dicts: {pct: bar_idx}.
    bar_idx = n_fwd+1 means never hit.
    """
...
def evaluate_config(signals_with_exits, target_pct, stop_pct, time_exit, wf_split):
    """
    Evaluate one (target, stop, time) config across a list of
    (signal, tgt_bars, stp_bars) tuples.
...
def _metrics(pnls):
    ...
    # Per-trade Sharpe annualized: mean/std * sqrt(trades_per_year)
    # Assume ~4.2 years of data (2022-01 to 2026-03)
    years = 4.2
    tpy = n / years
    if std > 1e-10:
...
def score_risk_budget(sharpe, win_rate):
    """Map (Sharpe, WR) to risk budget $10K-$150K."""
...
def main():
    t0 = time.time()
    print("=" * 80)
    print("  KITE VWAP Mean Reversion — Exhaustive Parameter Sweep")
    print("  Data: KITE Liberator minute bars, SPY, RTH only")
    print("  Methodology: frontside fills, 14-day Wilder ATR, session VWAP")
    print("=" * 80)

    # --- Load data ---
    print("\n[1/5] Loading minute data...")
    minute_data = load_minute_data()

    print("\n[2/5] Loading daily data and computing ATR...")
    ...
    for idx, d in enumerate(trading_days):
        ...
        if (idx + 1) % 200 == 0:
            print(f"  Day {idx+1}/{len(trading_days)}: {len(all_signals):,} signals")
    ...
    # For each signal group, precompute exits
    precomputed = {}  # key -> [(signal, tgt_bars, stp_bars), ...]
    total_sigs = 0
        ...
        for direction in DIRECTIONS:
            ...
            if count % 50000 < len(TARGET_PCTS) * len(STOP_PCTS) * len(TIME_EXITS):
                ...
                print(f"  {count:>9,}/{n_configs:,} "
                      ...
                      f"pos={positive:,} wf={wf_passed_count:,}")
    ...
    # Filter to configs where BOTH target and stop fire (proves real edge)
    # A config where stop never fires = degenerate (target too tight, stop too wide)
    ...
    print(f"  (Excluded {len(wf_passed) - len(wf_real):,} degenerate configs "
          f"where stop never fires)")

    # Use wf_real for all analysis
    wf_passed = wf_real

    if not wf_passed:
        print("\n  No configs passed walk-forward validation.")
        # Still show top positive configs
        ...
        for r in top_sharpe[:15]:
            ...
            # Use median entry price for sizing
            key = (r['entry_pct'], r['direction'])
            group = precomputed.get(key, [])
            prices = [s['entry_price'] for s, _, _ in group]
            med_price = np.median(prices) if prices else 500.0
...
def _serialize(r):
    ...
    return {k: v for k, v in r.items() if k not in ('fwd_h', 'fwd_l', 'fwd_c')}
