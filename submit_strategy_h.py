#!/usr/bin/env python3
"""
Submit Strategy H (KITE-informed vel tiers + TOD) backtest to KITE.
Uses gap-filtered batch files (strat_d_XX.txt) — 982 trading days.
"""
...
def submit_batch(batch_file, batch_idx):
    ...
    if not dates:
        return None
    cmd = [
        'kti', 'backtest', 'submit', STRATEGY_GUID,
        '--dates', dates, '--symbols', 'SPY',
...
def main():
    print('=' * 70)
    print('  Strategy H KITE Backtest')
    print('  KITE-informed vel (25/75/100/150/25K) + skip 30-45m + midday 1.5x')
    print(f'  {NUM_BATCHES} batches, 982 trading days (gap-filtered)')
    print('=' * 70)
