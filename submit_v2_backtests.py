#!/usr/bin/env python3
"""Submit V16, V16b, V9 backtests to KITE with rate limiting.
Uses strat_d_XX.txt batch files (40 batches, 982 dates).
3 strategies x 40 batches = 120 submissions.
"""
...
# Strategy configs: (name, sizing_mode)
STRATEGIES = [
    ("V16",  5),
    ("V16b", 6),
    ("V9",   7),
    ...
    for batch_idx in range(NUM_BATCHES):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                total_submitted += 1
                print(f"  [{total_submitted:3d}/120] {strat_name} batch {batch_idx:02d} -> {h}")
            else:
...
# Save hashes
with open("v2_submission_hashes.json", "w") as f:
...
print(f"  Hashes saved to v2_submission_hashes.json")
print(f"{'='*60}")
